...
8. **Derived render surfaces are views, not artifacts.** ⛔ **The private
   "internal read" page (`artifacts/read/index.html`, built by
   `theprojection render-read`) was retired 2026-08-25** (Ben: "that
   predates the actual website... kill it utterly") — it predated
   `theprojection.org` and had become a repeated size/Artifact-refusal
   fight for a reader nobody was using once the public site existed. The
   principle survives for what's left: `render_read.py`'s parsing/rendering
   helpers are reused, unversioned, by `readouts.py` and `publish/
   adapter.py` for the real public-site pipeline — those stay views over
   attention/ + digests + timelines, need no provenance sidecar, and
   deleting their output loses nothing.
