---
lens: mental-health
date: 2026-09-16
status: building
window_start: 2026-09-16T05:00:00-04:00
as_of: 2026-09-16T10:00:00-04:00
coverage: pending
---

# Mental Health — 2026-09-16

*Curated agentic-interim, 05:00 ET → ~10:00 ET Wednesday. Collector
pipeline not installed in this environment; this pass rests on
WebSearch/WebFetch against `attention/watchlist.yaml`, open threads, and
a ClinicalTrials.gov sweep. Confirmed thin — only Capital & corporate
cleared the bar; Product & market, Policy/regulation/legal, Research &
evidence and Clinical safety & harm all came back empty this window.*

## Today's throughline

Two open loose ends on `mh-clinical-infra-funding` resolved this morning.
Sword Health made its Headspace acquisition official — a signed
all-cash agreement, with close now targeted for "the beginning of Q4
2026" rather than the 09-14 date the original filing implied — and Aware
Recovery Care's previously-unnamed distress-sale buyer got a name: Renew
Health + Recovery, an existing 11-state in-home provider.

## Capital & corporate

- **Sword Health and Headspace made their deal official:** a signed
  agreement to acquire Headspace's parent company OrangeDot, an all-cash
  transaction (reported $200-300M), with the close now set for "the
  beginning of Q4 2026." This resolves `sword-headspace-acquisition-
  close-0914`, which had flipped to passed-silent on 09-14 within its
  3-day retro-flip grace window — not a silent close, just a later real
  close than the filing implied.
  ([Sword Health, primary](https://www.globenewswire.com/news-release/2026/09/16/3363038/0/en/sword-to-acquire-headspace-bringing-ai-care-to-the-world-s-most-trusted-mental-health-brand.html), [Fierce Healthcare](https://www.fiercehealthcare.com/digital-health/sword-health-acquire-headspace-all-cash-deal))
  <!-- k: t=mh-clinical-infra-funding axis=capital-corporate -->
- **Aware Recovery Care's previously-unnamed buyer has a name:** Renew
  Health + Recovery, an existing 11-state in-home substance-use treatment
  provider, has purchased "many of the assets" and clinical teams/models
  of the financially distressed Aware Recovery Care, with no stated
  interruption to existing clients.
  ([AP via Rutland Herald](https://www.rutlandherald.com/ap/state/renew-health-recovery-purchases-assets-of-aware-recovery-care/article_2193a02c-3e10-566b-ab1e-c72e0950934e.html))
  <!-- k: t=mh-clinical-infra-funding axis=capital-corporate -->

## 🧪 Clinical trials

None this window rising above a small single-site registration. One
noted for reference: NCT07823114, a Peking University feasibility study
of a 2-week smartphone-based ecological-momentary-intervention for
psychological distress in young adults with multiple adverse childhood
experiences — single-arm, not yet recruiting.

## ⏳ Upcoming & expected

**One retro-flip today (slipped); 7 pending in the next 7 days.**

- 🔄 **`sword-headspace-acquisition-close-0914` — slipped.** See Capital
  & corporate above; new target close ~2026-10-01 (Q4 2026).
- 🚧 **`anthropic-wellbeing-grants-deadline-0921` — due 2026-09-21 (5
  days).** Nothing new today.
- 🚧 **`raine-jccp-cmc-0923` — due 2026-09-23 (7 days).** Docket
  unchanged.
- 🚧 **`ca-ab1979/ab2575/sb903/sb503-governor-action` — all due
  2026-09-30 (14 days).** Confirmed still unsigned as of yesterday's
  check; no newer signing announcement found today.

## 🔄 Map changes

- `~ artifacts/threads/mh-clinical-infra-funding.md` — gained a
  2026-09-16 entry (both items above).
- `attention/upcoming.yaml`: `sword-headspace-acquisition-close-0914`
  retro-flipped from passed-silent to slipped, new due ~2026-10-01.
- 2026-09-15 finalized today: coverage critic found one genuine miss
  (Prosper Health's autism-specialty raise, and a former Discovery
  Behavioral CEO's new AI-monitored addiction venture) — folded into
  09-15's digest appendix and `mh-clinical-infra-funding`'s timeline.
  Full detail in `coverage-log.md`'s 2026-09-16 entry.

## 🧵 Thread candidates

**None today.** Both items land inside the existing
`mh-clinical-infra-funding` thread.

---

A thin morning with two real resolutions: Sword Health's Headspace
acquisition is now officially signed, closing Q4 2026 rather than the
date the original filing implied, and Aware Recovery Care's distress-sale
buyer finally has a name. Nothing else cleared the bar.
