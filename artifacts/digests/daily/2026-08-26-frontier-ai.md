...
**A stealth model that had been topping a usage leaderboard turned out
to be Chinese, and its maker said so voluntarily.** Zhipu/Z.AI confirmed
it built "Ox Alpha" — released uncredited over the weekend, first past
DeepSeek on OpenRouter usage — and said it would open the weights the
same night. **The map had carried "Ox Alpha" as an unresolved thread
candidate for two runs precisely because nobody would claim it**, so
today's confirmation retires the question rather than answering it in
the abstract: it was never an unattributable frontier model, it was a
Chinese lab choosing anonymity as a launch strategy and then dropping it
once the leaderboard position was established. Beyond that, the day's
AI news is water and land — Amazon withdrew a groundwater application in
Virginia after a state regulator said the aquifer could not take it.
...
- **Nvidia reports Q2 FY2026 after today's close** — the call is
  confirmed for 17:00 ET, covering the quarter ended 2026-07-26, with
  CFO commentary posting to the IR site shortly after results. Consensus
  clusters around $91-95bn of revenue and roughly $2.09 adjusted EPS.
  **This is the print the lens has been pointing at all week**, and it
  arrives one day after OpenAI published benchmark results claiming its
  own inference silicon beats Nvidia's flagship on performance-per-watt.
...
---
Zhipu/Z.AI claimed the stealth model that had been beating DeepSeek on
OpenRouter usage and said it would open the weights, resolving a thread
candidate this map had carried unattributed for two runs. Amazon pulled a
Virginia groundwater application after the state warned the aquifer could
not take it. Nvidia reports after the close, a day after OpenAI published
benchmarks claiming its own chip beats Nvidia's flagship on power.
