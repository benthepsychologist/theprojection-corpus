...
6. ✎✎✎ **cloud-researcher is proposing (unruled, 2026-08-27) to generalize §8.1's entity-reconciliation method into a shared `reconcile-entities` kit tool**, and separately notes pm has more mature graph machinery this design doesn't use — a stale-surface regen loop, an anti-HARKing lock (`predicted.logged_at` predates every evidence edge), predicate-whitelist conformance checks. Neither blocks execution: their own sequencing puts generalizing *after* this repo's step 1 lands, as the reference case. `graph/validate.py` (§8 intro) stays scoped to referential integrity for now; adopting the richer conformance layer is a later pull, not a rewrite, once the kit exists to pull from. Confirmed the same day: Ben's ruling that every line of inquiry gets a corpus names this repo's mapping explicitly — "AI buildout → theprojection-corpus: already true" (`cloud-researcher/INBOX/2026-08-27-pm-loi-corpus-contract.md`).
...
| A11 | **`collect --corpus` targeting** | flag silently inert; env-var workaround | bug fix | `/week` 4b's documented command works as written | ✅✅ **fixed 2026-08-27** — `cloud_researcher/collect.py` now uses `paths.corpus_root()`; verified both paths by hand on their side |
